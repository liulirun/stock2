...
class DbHelper:
    ...
    def _latest_date_in_db(self, stock_name):
        _query = (
            "SELECT stock_date FROM stock.{} ORDER BY stock_date DESC LIMIT 1".format(
                stock_name
            )
        )
    ...
    def insert_to_stock_table(self, stock_name):
        ...
        l_length = len(dl)
        stock_list = [
            (dl[i], pl[i], vl[i], m3[i], m13[i], m34[i], bl[i], ds, ds)
            for i in range(_index, l_length)
        ]
    ...
    def stock_data_for_days(self, table_name, days=484):
        _query = "SELECT * FROM stock.{} ORDER BY stock_date DESC LIMIT {};".format(
            table_name, days
        )
        result = self._fetch_all(_query)
        return result
    ...
    def stock_data_since_date(self, table_name, start_date, end_date=date.today()):
        _query = "SELECT * FROM stock.{} where stock_date >= '{}' and stock_date <= '{}' ORDER BY stock_date ASC;".format(
    ...
    def current_stock_cn_name(self, table_name):
        _query = (
            "SELECT cn_name FROM stock.cn_names WHERE cn_code='{}' LIMIT 1;".format(
                table_name
            )
        )
        result = self._fetch_all(_query)
    ...
    def closest_date(self, table_name, start_date):
        _query = "SELECT stock_date from stock.{} where stock_date >= '{}' \
            order by stock_date ASC Limit 1;".format(
