...
class Test_New(unittest.TestCase):
    ...
    def test_current_stock_cn_name_happy_path(self, mock_fetch_all):
        ...
        result = self.helper.current_stock_cn_name(table_name="mock_tets")
    ...
    def test_latest_date_in_db_happy_path(self, mock_fetch):
        ...
        result = self.helper._latest_date_in_db(stock_name="mock_tets")
...
class TEST_FUNC_INSERT_TO_STOCK_TABLE(unittest.TestCase):
    ...
    def test_insert_stock_data_to_db_empty(self, mock_read_json):
        ...
        result = self.helper.insert_to_stock_table(stock_name="mock_tets")
        self.assertEqual(len(result), 0)

    @mock.patch("data.db_helper.DbHelper._read_json")
    @mock.patch("data.db_helper.DbHelper._latest_date_in_db")
    @mock.patch("data.db_helper.DbHelper.insert_tick_data_to_table")
    @mock.patch("data.db_helper.DbHelper.truncate_table")
    def test_insert_stock_data_to_db_happy_path(
        self,
        mock_read_json,
        mock_db_latest_date,
        mock_insert_tick_data,
        mock_truncate_table,
    ):
        self.json["sp"] = [1.0, 1.0, 2.0]
        self.helper._read_json.return_value = self.json
        self.helper._latest_date_in_db.return_value = date(2012, 1, 1)

        result_list = self.helper.insert_to_stock_table(stock_name="mock_tets")

        self.assertEqual(self.helper.insert_tick_data_to_table.called, True)
        self.assertEqual(self.helper.insert_tick_data_to_table.call_count, 1)
    ...
    @mock.patch("data.db_helper.DbHelper.insert_tick_data_to_table")
    @mock.patch("data.db_helper.DbHelper.truncate_table")
    def test_insert_stock_data_to_db_split_list_none_called(
        self,
        mock_read_json,
        mock_db_latest_date,
        mock_insert_tick_data,
    ...
    @mock.patch("data.db_helper.DbHelper.insert_tick_data_to_table")
    @mock.patch("data.db_helper.DbHelper.truncate_table")
    def test_insert_stock_data_to_db_split_list_called_only_once(
        self,
        mock_read_json,
        mock_db_latest_date,
        mock_insert_tick_data,
    ...
    @mock.patch("data.db_helper.DbHelper._latest_date_in_db")
    @mock.patch("data.db_helper.DbHelper.insert_tick_data_to_table")
    def test_insert_tick_data_not_being_called(
        self, mock_read_json, mock_db_latest_date, mock_insert_tick_data
    ):
        self.json["sp"] = [1.0, 1.0, 1.0]
        self.helper._read_json.return_value = self.json
        self.helper._latest_date_in_db.return_value = date(2012, 1, 3)

        result_list = self.helper.insert_to_stock_table(stock_name="mock_tets")

        self.assertEqual(self.helper.insert_tick_data_to_table.called, False)
        self.assertEqual(len(result_list), 0)

    @mock.patch("data.db_helper.DbHelper._read_json")
    @mock.patch("data.db_helper.DbHelper._latest_date_in_db")
    @mock.patch("data.db_helper.DbHelper.insert_tick_data_to_table")
    def test_insert_stock_data_to_db_no_data_in_DB(
        self, mock_read_json, mock_db_latest_date, mock_insert_tick_data
    ):
        self.json["sp"] = [1.0, 1.0, 1.0]
        self.helper._read_json.return_value = self.json
    ...
    @mock.patch("data.db_helper.DbHelper._latest_date_in_db")
    @mock.patch("data.db_helper.DbHelper.insert_tick_data_to_table")
    def test_something_wrong_with_Database_exception(
        self, mock_read_json, mock_db_latest_date, mock_insert_tick_data
    ):
        self.json["sp"] = [1.0, 1.0, 1.0]
        self.helper._read_json.return_value = self.json
        ...
        with self.assertRaises(ValueError) as context:
            self.helper.insert_to_stock_table(stock_name="mock_tets")
    ...
    @mock.patch("data.db_helper.DbHelper._latest_date_in_db")
    @mock.patch("data.db_helper.DbHelper.insert_tick_data_to_table")
    def test_DB_will_have_gap_exception(
        self, mock_read_json, mock_db_latest_date, mock_insert_tick_data
    ):
        self.json["sp"] = [1.0, 1.0, 1.0]
        self.helper._read_json.return_value = self.json
